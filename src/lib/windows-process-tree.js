...
async function terminateUntilGone(child, {
  ...
  while (true) {
    ...
    if (now >= nextKillAt) {
      ...
      try {
        treeKillConfirmed = await killTree(pid);
        reportKillAttempt(onKillAttempt, { phase: 'tree', result: treeKillConfirmed });
      } catch (error) {
        reportKillAttempt(onKillAttempt, { phase: 'tree', error });
      }
      if (!treeKillConfirmed && !isClosed() && await processIdentityAlive(identity)) {
        try {
          const result = child.kill('SIGKILL');
          reportKillAttempt(onKillAttempt, { phase: 'force', result });
        } catch (error) {
          reportKillAttempt(onKillAttempt, { phase: 'force', error });
        }
      }
...
async function terminateWithoutReusablePid(child, { isClosed, wait, pollMs, onKillAttempt }) {
  try {
    const result = child.kill('SIGKILL');
    reportKillAttempt(onKillAttempt, { phase: 'force', result });
  } catch (error) {
...
function reportKillAttempt(onKillAttempt, detail) {
  try { onKillAttempt?.(detail); } catch {}
}
