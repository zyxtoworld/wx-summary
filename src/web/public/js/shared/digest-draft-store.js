...
const DIGEST_DRAFT_RENDER_THEMES = new Set(['auto', 'light', 'dark']);
const DIGEST_DRAFT_RENDER_FONT_SIZES = new Set(['normal', 'large']);
...
function normalizedRenderOptions(value = {}) {
  const source = value && typeof value === 'object' && !Array.isArray(value) ? value : {};
  const theme = boundedText(source.theme, 16);
  const fontSize = boundedText(source.font_size || source.fontSize, 16);
  return {
    theme: DIGEST_DRAFT_RENDER_THEMES.has(theme) ? theme : 'auto',
    font_size: DIGEST_DRAFT_RENDER_FONT_SIZES.has(fontSize) ? fontSize : 'normal',
  };
}
...
function normalizedDraft(value = {}, { savedAt = Date.now(), accountFingerprint = '' } = {}) {
  ...
  return {
    ...
    render_options: normalizedRenderOptions(source.render_options),
  };
}
...
export function readDigestDraftSnapshot(storage, storageKey, scopeIdentity, {
  ...
  try {
    ...
    if (expectedFingerprint !== draft.account_fingerprint) {
      return { ok: true, draft: null };
    }
...
export function digestDraftHasMeaningfulInput(value = {}) {
  ...
  const renderOptions = draft.render_options;
  return draft.selected_group_ids.length > 0
    ...
    || renderOptions.font_size !== 'normal';
}
