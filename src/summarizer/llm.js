...
async function callJsonModel({ settings, model, groupName, since, until, messageBundle, mode, signal = null }) {
  ...
  const system = [
    ...
    mergeMode ? '当前输入是全量请求失败后的多个分段 JSON 摘要。你正在合并分段摘要，必须综合所有分段；不要因为后段覆盖前段而丢掉链接、待办、参与人、来源时间或需要跟进议题。links/todos/topics 要从各分段去重保留，冲突时合并信息而不是删除。' : '',
  ].join('\n');
  const intro = [
    `群名：${groupName}`,
    `时间窗：${since} ~ ${until}`,
    ...
    '请按公共群纪要视角提炼真正有用的信息，保留明确的待办、重要网页链接及其用途说明、需要跟进的议题。',
  ].filter(Boolean).join('\n');
  const user = [
    intro,
    '',
